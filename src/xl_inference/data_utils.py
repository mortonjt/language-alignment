...
class Corpus:
    def __init__(self, path, dataset, use_bpe, *args, **kwargs):
        ...
        if self.dataset in ['uniref50']:
            self.vocab.count_file(os.path.join(path, 'train.txt'))
            self.vocab.count_file(os.path.join(path, 'valid.txt'))
            self.vocab.count_file(os.path.join(path, 'test.txt'))
        elif self.dataset == 'bfd':
            self.vocab.count_file(os.path.join(path, 'vocab.txt'))
            #self.vocab.count_file(os.path.join(path, 'bfd_test.txt'))
            train_path_pattern = os.path.join(path, 'bfd_train_*.txt')
            train_paths = glob.glob(train_path_pattern)
            assert train_paths, f'Nothing found at {train_path_pattern}' 
        ...
        if self.dataset in ['uniref50']:
            ...
            self.train = sorted(train_paths)
            self.valid = self.vocab.encode_file(
                os.path.join(path, 'bfd_valid.txt'), ordered=True, add_eos=False,add_double_eos=True)
            self.test = self.vocab.encode_file(
                os.path.join(path, 'bfd_test.txt'), ordered=True, add_eos=True,add_double_eos=True)
        elif self.dataset in ['ptb', 'wt2', 'wt103']:
            self.train = self.vocab.encode_file(
    ...
    def get_dist_iterator(self, split: str, rank: int, max_rank: int, *args, **kwargs):
        ...
        #print('debuging why files not loading aligned')
        #print('data is:')
        #print(data)
        #print('------------------')
        #print('subset is')
        ...
        #print('------------------')
        if len(subset) == 0:
            print('One of the machines do not have any data to process - make sure there is enough data for all machines')
            sys.exit() 
        if self.dataset in ['lm1b', 'wiki','bfd']:
            if split == 'train':
                #print('start training and calling LMMultiFileIterator')
                #for file in subset:
                    #copyfile(file, '/mnt/bb/deepforce/')
                    #subprocess.run(["jsrun", "-n 1 cp " + file + " /mnt/bb/deepforce/"])
                #subset = glob.glob("/mnt/bb/deepforce/bfd_train_*.txt")
...
def get_lm_corpus(datadir: str, dataset: str, use_bpe=False, max_size=None) -> Corpus:
    ...
    if os.path.exists(cache_filepath) and dataset != 'wiki' and dataset != 'bfd':
        print('Loading cached dataset...')
        corpus = torch.load(cache_filepath)
    else:
        print('Producing dataset {}...'.format(dataset))
        kwargs = {'max_size': max_size}
        if dataset in ['uniref50']:
            kwargs['special'] = ['<eos>']
            kwargs['lower_case'] = False
        elif dataset in ['bfd']:
            kwargs['special'] = ['<S>']
            kwargs['lower_case'] = False
        elif dataset in ['wt103', 'wt2', 'wt103-normal']:
            kwargs['special'] = ['<eos>']
            kwargs['lower_case'] = False
        elif dataset == 'ptb':
            kwargs['special'] = ['<eos>']
        ...
        corpus = Corpus(datadir, dataset, use_bpe=False, **kwargs)
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='text8',
                        choices=['ptb', 'wt2', 'wt103', 'lm1b', 'enwik8', 'text8', 'wt103-normal', 'wiki', 'uniref50','bfd'],
                        help='dataset name')
